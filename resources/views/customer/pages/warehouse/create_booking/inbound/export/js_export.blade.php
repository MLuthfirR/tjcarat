...
<script>
    ...
    function storeOrder() {
        ...
        if ($('#form_createorder').parsley().isValid({group: "main"}) && requirement_fulfilled) {
            ...
            jQueryAjax(
                '{{ route("webapi.warehouse.booking.store.inbound") }}',
                'POST',
                mainForm,
                function (data) {
                    // Success
                    ...
                    $('#success-modal').find('.button-body').html(`<a href="{{ route('warehouse.index') }}" class="btn btn-outline-primary">Continue</a>`)
