@push('scripts')
    <script>
        ...
        function finalizeContainerOrder() {
        ...
        function finalizeCargoOrder() {
            var cargo_json_array = $('#cargo_list_form').serializeArray();
            if (cargo_json_array.length === 0) {
                addToast('Error: No cargo selected', 'text-danger', 'Please select 1 or more cargo to proceed with the order.');
            } else {
                populateFinalTable(
                    cargo_json_array.map(function (item) {
                        return JSON.parse(item.value)
                    })
                );
                fetchPaidthruPrice();
        ...
        function toggleInputSPPBDocument(delay=0) {
            disableSelectBtn();
            $('.doc-collapse').collapse('hide');
            setTimeout(() => { $('#document-input-collapse').collapse('show'); }, 350);
        }

        function checkSPPBDocument(bl_no, bl_date) {
            disableSelectBtn();
            $('.doc-collapse').collapse('hide');
        ...
        function populateSelectTableCargo() {
        }
        ...
        function insertCargoToTable(data) {
            if (data) {
                $('#cargo-list-table-container > table > tbody').append(`
